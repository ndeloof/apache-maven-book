...
image:Maven3.png[Maven 3,align="center"]
...
.Figure 02-04
image:02-04.png[align="center"]

Contenu du dépôt local pour la bibliothèque jcr-on-cassandra.

image:MangaVincentM.png[float="left"]

Au labo, Vincent analyse nos indices. Le fichier _maven.repositories_
...
<repository>
    ...
    <layout>format du dépôt, « legacy » pour les dépôts Maven 1 antédiluviens, non supportés par Maven 3</url>
</repository>
-------------------------------------------------------------------------------
...
[NOTE]
====
Maven 2 ne comporte pas ce mécanisme et n'aurait pas bronché. Cela aurait
...
Pardon ? Maven 2 est compatible uniquement avec Java 1.3 ? Pas de
...
utilisé par un développeur, le compilateur utilisé par Maven 2 est
configuré, par défaut, pour cette valeur 1.3, et pour Maven 3 par défaut à
...
Maven 2 ne fait pas de séparation très claire entre les dépôts
...
Java, dont le plus connu d'entre eux est jUnit footnote:[Il existe deux variantes majeures de jUnit, jUnit3 et jUnit4, la seconde utilisant la syntaxe Java 5 que nous avons retenue pour notre exemple.].
...
nouvelle API `java.nio.file` de Java 7.
...
Husdon, lui aussi en train de surveiller la construction de Maven 2, dans
...
De nombreux dépôts de bibliothèque Maven existent en version « Maven 1 » ou
« Maven 2 ». Le premier format est conservé pour des raisons historiques.
Le second présente une structure plus hiérarchisée et des métadonnées
supplémentaires. Maven 2 sait utiliser les deux formats. Le dépôt
_central_, qui pointe physiquement sur les mêmes fichiers, est
accessible au format Maven 1 footnote:[http://repo2.maven.org/maven/]
...
significative dans sa mouture JavaEE 6. Maven ne pouvait pas faire
...
comprenne le rôle dans l'architecture. Jusqu'à l'émergence de JavaEE 5 et
de JavaEE 6, qui les dépoussièrent sensiblement, leur utilisation était
...
travaille de son côté sur une application JavaEE 6, et les évolutions
...
Les nouveautés de JavaEE 6 vont bien au-delà de ces quelques
...
JavaEE 6) sans avoir besoin d'assembler notre WAR : il nous suffit de
...
disposition de celui-ci la bibliothèque de notre outil gea-check sous
...
comparable, en dehors du mécanisme d'annotations qui jusqu'il y a peu de
...
API historique, alors qu’en interne les choses ont beaucoup changé.

Dans l’équipe, nous utilisons tous Maven 3, Java 7, JavaEE 6, pourquoi
s’encombrer de ce passé obsolète ?

Olivier s’attaque donc à moderniser notre socle technique. Bye Bye
Maven 2, ce qui se traduit dans note POM par un prérequis sans appel :
...
[source, java]
.Listing 11.6 : Le mojo _GEACheck_ en version Maven 3 / JSR-330
-------------------------------------------------------------------------------
/**
 * Tâche de génération de la documentation au format Geegol.
 */
...
tout de même notre fierté, et _noubliepaslalistedescourses_ reste notre
bébé !
...
d'atteindre ce taux de 75 %. Le développeur étant par nature feignant,
...
d’une version à l’autre sans douleur. Joli tour de force ! Finalement, la
configuration de nos rapports s’effectue donc comme l’indique le listing 12.2, et nous permet d’associer au site web du projet la documentation
javadoc et le code source du projet, navigable sous forme de pages web
(« Cross Reference »).

[source, xml]
.Listing 12.2 : Configuration de nos plugins de production de rapports
-------------------------------------------------------------------------------
  <build>
    <plugins>
      <plugin>
...
* ou encore, via un module d'extension, en exploitant tout autre
...
dans tout un ensemble de formats qu'il reste à imaginer (voir Figure 12-01).
...
Malgré nos efforts, et même si le répertoire target contient bien finalement
 un fichier
...
impossibles à reproduire et donc à corriger. Le passé a dû laisser à
...
Avec tous nos efforts sur notre projet _noubliepaslalistedescourses_ pour
...
nôtre, _malistedecadeauxpournoël_. Raphaël se voit offrir la lourde tâche
...
finalement par une petite guerre d'influence et de désinformation :

_JSF c'est naze,_

_GWT c'est super lourd,_

_Struts 2 c'est _has been_.
...
qui s'exécuterait _après_ la construction des modules. Ce problème,
...
développeurs. Le mécanisme de test de ce projet est ultraspécifique,
les artefacts construits – si, finalement, ils sont des archives JAR –
...
_This POM cannot be used to build Spring; it is a work in progress and
should only be used as part of a Maven repository upload bundle using
artifacts created by the spring build system._
...
les mêmes procédures. EasyAnt n'est cependant pas aussi _easy_ qu'il le
...
_Domain Specific Language_ (DSL), un langage de programmation dédié à une
...
dans le build. Par ailleurs, Gradle est un outil encore très jeune et
...
vous tend la perche pour accumuler les astuces et autres bizarreries qui
...
le développent et dont nous dépendons finalement.
...
que vous tenez entre les mains, auquel vous pouvez ajouter le _Definitive
Guide_ et _Better Build with Maven_, tous deux disponibles en ligne
gratuitement, et _Apache Maven 2 Effective Implementations_, proposé en
...
_software factory_, qui vise à proposer une solution industrialisée,
...
votre logo _Impossible Mission Force_.
...
et encore sous réserve que des développeurs prennent le temps de se
pencher à son chevet ! Maven 3.x est la branche active qui bénéficie de
...
vous pousse à continuer d’utiliser Maven 2.x, n'attendez rien de nouveau
de ce côté. Cependant, pour nos lecteurs qui seraient contraints à cette
...
l'enrichir et le corriger à nos dépens).
...
L'un des plus gros chantiers de Maven 3, et qui explique en grande partie
...
plupart de ses cas d'utilisation. Projets simples, multimodules,
plugins plus ou moins complexes, sont ainsi exécutés en boucle par un
harnais de sécurité antirégression de plus de 450 tests. Ce n'est pas
...
Eclipse, c’est-à-dire ne procéder à la reconstruction du projet que
...
Cette évolution vise un public très précis : ceux qui n’utilisent pas
...
possibilités d’extension qui lui sont propres, Polyglot essaie de
...
suffira d’obtenir depuis le dépôt la version associée du modèle. On
...
de mixins ; un mixin pour la gestion de la persistance Hibernate, un
...
intégré en miroir sur un compte GitHub. Sonatype utilise
...
Apache. Ainsi, si ces annonces concernent avant tout les outils de
...
D’une part, la généralisation de l’utilisation des annotations Java 5 et
...
à chaque nouveau build. L’IDE pourrait tirer parti de ces évolutions,
...
Cette évolution permettra à terme de créer des passerelles entre les
...
dépôt. Cette API remonte à Maven 2 et a été dérivée pour supporter HTTP,
...
travailler sous le capot pour en améliorer les performances.
...
L'évolution du développement est rapide et peu documentée, il est donc
...
Sisu footnote:[http://sisu.sonatype.org], qui permet à Maven 3 de fonctionner sur la base
...
sont à sa base ne sont plus des idéalistes barbus en sandales-chaussettes, mais des professionnels pragmatiques.
...
Maven nous a accompagnés pendant ces 14 chapitres en répondant à chaque
fois à nos attentes. Nous avons dû parfois faire appel à des plugins
...
lui développant une plateforme serveur dédiée.
...
corde à notre arc : le petit projet _n’oublie pas la liste des courses_ a
...
applications de gestion ou de modélisation sur un socle reconnu et avec
...
plugin, Eclipse PDE (Platform Development Environment) et tente d’y
...
retenue, c’est de ne pas utiliser Maven… Indignation, hurlements, la
...
finissons donc par l’écouter. Son argumentaire est le suivant :
...
Tycho est un des nombreux projets satellites de Maven, sur lequel
Sonatype a énormément investi. L’idée de Tycho est de couper la poire
...
[NOTE]
====
Tycho ne fonctionne donc qu'avec Maven 3. Le lourd travail de refonte
...
Si vous êtes contraint d’utiliser Maven 2, par exemple pour respecter des
...
et du legacy (dépôts Maven 1, obsolètes et plus supportés par Maven 3),
...
dans le projet de plugin. À partir de cela, il constitue le classpath et
...
Pascal nous a bluffés, et même si nous avons encore un peu de mal à
...
bibliothèque technique de Geegol pour une rapide mise à niveau histoire
d’avoir l’air moins bêtes la semaine prochaine.

Quelques semaines plus tard, le travail de Pascal a bien avancé, et il
nous présente maintenant son build complet, qui inclut désormais la
...
relevés avec panache, merci Pascal !
...
techniques de compression, le runtime Java 6 pèse 14 Mégaoctets (sans
...
10 ans ! Quant à Arnaud, il s’interroge sur le fameux « mécanisme » que
...
bibliothèques, P2 avec les bundles, et il semble peu probable que Java 8 incorpore Tycho.
...
Dans un premier temps, les travaux ont amené à envisager un nouveau
...
En tant qu’utilisateurs de Maven, nous pouvons dire que notre code
...
très spécifique des mainteneurs de Java. Avec huit versions de la
...
JAR dans votre application web ! Sur ce terrain, Maven ne peut pas grand-chose pour nous, à part espérer que les métadonnées qu’il exploite
soient correctes.
...
Google pour la plateforme Android, afin de ne pas surconsommer les
...
un environnement très limité en ressources.
...
une fois pour toutes. Ici encore, la vitesse de démarrage de Java pourra
ainsi être améliorée. Cumulée avec les autres techniques qu’il nous a
indiquées, Mark nous annonce un gain sur le temps de chargement pouvant
atteindre 70 % !
...
Nous sommes tous très dubitatifs sur ce que JigSaw va donner sur notre
écosystème et notre outillage. Il faudra déjà attendre une forte
adoption de Java 8, puis la mise à jour des frameworks majeurs que sont
...
Voici une URL que nous connaissons bien, celle du dépôt Maven central !
...
on les avait approchés initialement, et aboutissent à des solutions
...
L'activité de _noubliepaslalistedescourses_ ne cessant de croître, nous
...
* de simplifier l'utilisation de Maven ;
...
pourtant pas une fatalité, et cela a été démontré dans _Better Builds
With Maven_ (disponible librement en ligne), qui propose un POM
...
qui compte finalement. Par contre, nous gagnerons dans la finesse de nos
...
[source, xml]
.Listing 16.2 un profil pour activer le cycle de vie reconfigurable de m2eclise 0.9.9
-------------------------------------------------------------------------------
<profile>
      <id>m2e</id>
      <activation>
...
de pensée de Maven. Finalement, vos projets n'en seront que plus clairs et
...
central, il présente le projet _Geegol Shopping List_ dont la réussite est
...
Pour conclure, sachez que le _Definitive Guide_, ouvrage communautaire
...
je bosse sur le cœur de Sonar, mais aussi sur les plugins C#, voire...
...
des équipes de recherche, en France, en Espagne, au Danemark et au
Canada. Actuellement, je ne fais plus la recherche moi-même, je suis
dans l'administration de l'Académie des Sciences Slovaque à Bratislava
...
ses 5 ans), boit maintenant de l'alcool (Eh oui, tout chose arrive !),
...
Plus tard, j'ai participé aux longues discussions de design sur Maven 2…
...
(_intra-muros_, bien sûr), où j'ai commencé à enseigner au Cnam puis
...
J’ai 30 ans et je travaille depuis trois ans chez Zenika en tant que
...
Ses principaux centres d'intérêts et compétences sont Maven, Eclipse RCP,
...
geeks comme nous, sortir [line-through]*la plume*, le traitement de texte (en plus celui de Bilou !) et
...
l'ensemble des livraisons. Il existe beaucoup d'autres listes de
diffusions en fonction des sous-projets de Maven footnote:[http://maven.apache.org/mail-lists.html]. Enfin, en cas de besoin urgent, vous pouvez essayer de contacter une
...
modifier, de diffuser, et même de vendre le logiciel comme bon vous
...
Développé par Sonatype, en open source mais en dehors de la fondation
Apache, Aether encapsule la gestion de dépendance et la gestion des
dépôts d’artefact, indépendamment de leur implémentation. Maven propose
...
est réalisée par une classe Mojo, acronyme pour _Maven Old Java Object_
...
Conteneur historique de Maven 2, implémentant l’injection de dépendance
comme son concurrent Spring, mais n’ayant pas connu le succès de ce
dernier, sans doute en raison d’une documentation inexistante et d’un
manque de communication. Techniquement dépassé, il est remplacé dans
Maven 3 par Google Guice.
...
partir de fichiers XML, qui peuvent s'enrichir mutuellement via un
...
qualifié de _release_, ce que nous avons traduit par livraison. De
...
historique de Maven 2) dans un conteneur conforme à la norme JSR330, en
...
Snapshot. Cette option consomme évidemment plus d'espace disque sur le
...
* http://www.sonatype.com/products/maven/documentation/book-defguide. Le fameux _definitive guide_, en enrichissement permanent par les équipes de Sonatype.
